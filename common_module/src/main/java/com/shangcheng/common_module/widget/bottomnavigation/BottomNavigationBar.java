...
import com.shangcheng.common_module.utils.CommonUtils;
import com.shangcheng.common_module.widget.bottomnavigation.behaviour.BottomNavBarFabBehaviour;
import com.shangcheng.common_module.widget.bottomnavigation.behaviour.BottomVerticalScrollBehavior;
...
public class BottomNavigationBar extends FrameLayout {
    ...
    public ArrayList<BottomNavigationItem> mBottomNavigationItems = new ArrayList<>();
    public ArrayList<BottomNavigationTab> mBottomNavigationTabs = new ArrayList<>();
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mActiveColor = typedArray.getColor(R.styleable.BottomNavigationBar_bnbActiveColor, Color.RED);
            ...
            mActiveColor = CommonUtils.fetchContextColor(context, R.attr.colorAccent);
    ...
    private void init() {
        ...
        View parentView = inflater.inflate(R.layout.basecommon_bottom_navigation_bar_container_item, this, true);
...
//    }
    ...
    public void setUnReadCount(int index, int count) {
        if (count == 0) {
            mBottomNavigationTabs.get(index).fixed_bottom_navigation_un_read.setVisibility(GONE);
            mBottomNavigationTabs.get(index).view_un_read.setVisibility(GONE);
        } else if (count == -1) {
            mBottomNavigationTabs.get(index).fixed_bottom_navigation_un_read.setVisibility(GONE);
            mBottomNavigationTabs.get(index).view_un_read.setVisibility(VISIBLE);
        } else {
            mBottomNavigationTabs.get(index).fixed_bottom_navigation_un_read.setVisibility(VISIBLE);
            mBottomNavigationTabs.get(index).view_un_read.setVisibility(GONE);
        }
        mBottomNavigationTabs.get(index).fixed_bottom_navigation_un_read.setText(count + "");
    }
     ...
     */
    public void selectTabInternal(int newPosition, boolean firstTab, boolean callListener, boolean forcedSelection) {
        ...
        if (mTabSelectedListener != null) {
            if (forcedSelection) {
                ...
                if (oldPosition == newPosition) {
                    mBottomNavigationTabs.get(newPosition).selectAnimation();
