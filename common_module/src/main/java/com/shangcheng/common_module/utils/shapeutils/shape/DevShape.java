package com.shangcheng.common_module.utils.shapeutils.shape;
...
public class DevShape implements IDevUtils<Drawable, View> {
    ...
    private static DevShape mDevShape;
    ...
    public DevShape solid(@ColorRes int backgroundColorResId) {
        ...
        this.backgroundColor = backgroundColorResId;
        return this;
    }
     ...
     * @return BaseShape
     */
    public DevShape size(int width, int height) {
        this.mWidth = width;
        this.mHeight = height;
        return this;
    }
    ...
    public DevShape dashLine(int dashLineWidth, @ColorRes int dashLineColorResId, float dashWidth, float dashGap) {
        ...
        this.dashLineColor = dashLineColorResId;
        return this;
    }
     ...
     * 渐变样式
     * 默认 线性渐变 GradientDrawable.LINEAR_GRADIENT
     * 默认方向 从左到右渐变 GradientDrawable.Orientation.TOP_BOTTOM
     *
     * @param startColor 渐变开始端颜色 例：R.color.colorPrimary
     * @param endColor   渐变结束端颜色 例：R.color.colorPrimary
     * @return BaseShape
     */
    public DevShape gradient_right(@ColorRes int startColor, @ColorRes int endColor) {
        this.isGradient = true;
        this.gradientColors = new int[2];
        this.gradientColors[0] = DevShapeUtils.getContext().getResources().getColor(startColor);
        this.gradientColors[1] = DevShapeUtils.getContext().getResources().getColor(endColor);
    ...
    private Drawable createShape() {
        ...
        if (isLine) {
            // 设置边框线
            drawable.setStroke(dip2px(lineWidth), lineColor);
        }

        if (isDashLine) {
            //设置虚线
            drawable.setStroke(dip2px(dashLineWidth), dashLineColor, dip2px(dashWidth), dip2px(dashGap));
        }
        ...
        if (isGradient) {
            switch (gradientType) {
                case GradientDrawable.LINEAR_GRADIENT:
                    //设置线性渐变方向
                    drawable.setOrientation(createGradientOrientation());
                    break;
