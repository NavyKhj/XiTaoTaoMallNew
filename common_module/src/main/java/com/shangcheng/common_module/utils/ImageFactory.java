...
public class ImageFactory {
     ...
     * @throws FileNotFoundException
     */
    public static void storeImageNoRecycle(Bitmap bitmap, String outPath) throws FileNotFoundException {
        FileOutputStream os = new FileOutputStream(outPath);
        bitmap.compress(Bitmap.CompressFormat.JPEG, 70, os);
//        bitmap.recycle();
    }
     ...
     * @return
     */
    public static Bitmap ratio(String imgPath, float size) {
        BitmapFactory.Options newOpts = new BitmapFactory.Options();
        ...
        // 缩放比。由于是固定比例缩放，只用高或者宽其中一个数据进行计算即可,取宽和高较小的做缩放比
        int be;// be=1表示不缩放
        if (w > h && h > size) {// 如果高度小的话根据高度固定大小缩放
            be = (int) (h / size);
        } else if (w < h && w > size) {// 如果宽度小的话根据宽度固定大小缩放
            be = (int) (w / size);
        } else {//宽高相等
            be = (int) (w / size);
        }
     ...
     * @param outPath       输出路径
     * @param size          压缩尺寸
     * @param needsDelete   是否需要删除
     * @throws FileNotFoundException
     */
    public static void ratioAndGenThumb(String imgPath, String outPath, float size, boolean needsDelete)
            throws FileNotFoundException {
        int degree = readPictureDegree(imgPath);

        //图片压缩
        Bitmap bitmap = ratio(imgPath, size);
